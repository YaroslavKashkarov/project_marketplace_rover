...
export class SignInFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.signInForm.valueChanges.pipe(debounce((v) => interval(300))).subscribe(() => {
      if (this.signInForm.valid && this.signInForm.touched) this.signInForm.setErrors(null);
    });
  }
      ...
      this.authService.registerUser(registerUser).subscribe({
        ...
        error: (errorResponse: HttpErrorResponse) => {
          ...
          errorsArray.forEach((x) =>
            this.signInForm.get(x.field)?.setErrors({
              serverError: x.error,
            }),
          );
